...
export async function redeem(userId: string, rawCode: string): Promise<{ ownerUserId: string; code: string }> {
  ...
  /* «به دارندهٔ کد یک بلیط سبز داده بشه» — to the OWNER, not to the person who
     typed it. Best-effort on purpose: the claim above is already recorded, and
     failing the sign-up because a ticket did not land would be the wrong
     trade. A ticket that fails here is a support ticket, not a lost account. */
  try { await grantTickets(owner, REFERRAL_REWARD_TIER, REFERRAL_REWARD_COUNT); } catch { /* recorded either way */ }
