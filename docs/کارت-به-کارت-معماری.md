...
**وضعیت فعلی:** A2 پاسخ گرفت (سه نمونهٔ واقعی) و A3 باطل شد. بقیه باز است.
مرحله‌های ۱ تا ۶ به هیچ‌کدام وابسته نیستند و جلو می‌روند.

دو موردِ قرمز از بقیه فوری‌ترند: **A13** (واحد مبلغ رفاه) چون حدس زدنش خطای
ده‌برابری است، و **A16** (بلیط‌ها زیر آستانه‌اند) چون یک تصمیم محصولی است نه
فنی، و تا گرفته نشود معلوم نیست کارت‌به‌کارت اصلاً چه چیزی را می‌فروشد.
...
| **A4** | سرشمارهٔ فرستندهٔ هر بانک ثابت است | allowlist مبدأ به آن وابسته است | ⏳ **هنوز لازم است**: نمونه‌ها بدون شمارهٔ فرستنده آمدند. برای هر سه بانک، شمارهٔ فرستندهٔ پیامک را بفرستید |
...
| **A13** 🔴 | مبلغ پیامک **رفاه** ریال است یا تومان | پیامک رفاه هیچ واحدی نمی‌نویسد. حدس زدن یعنی خطای ده‌برابری در تطبیق | یک واریز با مبلغ معلوم به حساب رفاه، و اینکه پیامکش چه عددی نوشت |
| **A14** | یک نمونهٔ **برداشت** از هر سه بانک | قاعدهٔ ذخیرهٔ الگو (بند ۱۲-۳) نمونهٔ منفی اجباری می‌کند؛ مخصوصاً رفاه که واریز و برداشتش فقط با یک علامت فرق دارند | یک پیامک برداشت/خرید از هر بانک |
...
| **A16** 🔴 | بلیط‌ها زیر آستانه‌اند و با کارت‌به‌کارت خریدنی نیستند | مستقیماً روی درآمد می‌نشیند (بند ۳-۲) | تصمیم شما: بستهٔ چندتایی بلیط، یا فقط آیتم‌های بالای ۵۰٬۰۰۰ تومان، یا صبر تا درگاه بانکی |
...
| مرحله | محتوا | چرا اول |
|---|---|---|
| **۱ — F2** | جدول بادوام `order_fulfilments`؛ `_fulfilled: Set` و `_seen: Map` **حذف** می‌شوند | تحویل دوباره را می‌بندد. به هیچ سؤال بازی وابسته نیست، و کارت‌به‌کارت بدون آن اصلاً امن نیست |
| **۲ — F3** | `order_fulfilments` به‌عنوان **منبع سوم درآمد** در `accountingService`، با تفکیک بلیط / فروشگاه / کمک‌ها | درآمد درگاه از روز اول غایب بوده؛ قبل از اینکه کانال اصلی شود بسته می‌شود. به `wallet_ledger` نوشته نمی‌شود |
| **۳ — F4 + F7** | حذف `POST /v1/payments/intents` و `/verify`؛ حذف `#topupSheet`، `selectTopupAmount`، بدنهٔ `submitTopup` و کارت «شارژ سریع» | `#topupSheet` دقیقاً همان‌جایی است که ورودی پرداخت تازه می‌نشیند. تمیز کردنش قبل از ساختن، نه بعدش |

### خودِ کارت‌به‌کارت

| مرحله | محتوا | خروجی قابل مشاهده |
|---|---|---|
| **۴** | `availability` سه‌حالته + `card_to_card` در `GATEWAY_TYPES` + `c2c_cards` + `money.ts` + `amountAllocator` + تب «کارت‌های مقصد» | ادمین کارت تعریف می‌کند؛ درگاه بانکی «به‌زودی» می‌شود |
| **۵** | `sessionService` + `orders/quote`/`orders/pay` تازه + صفحهٔ پرداخت کلاینت | کاربر صفحهٔ واریز را می‌بیند؛ تسویه هنوز فقط دستی |
| **۶** | پنل: صف تراکنش، ثبت دستی پیامک، تخصیص دستی، تسویه | زنجیره **کامل کار می‌کند** با ورود دستی — قبل از یک خط کد اندروید |
| **۷** | `bankSms` ingest + کامپایلر قالب + تب الگوهای بانک + `matchService` + `c2cWorker` | تطبیق خودکار روی پیامکی که دستی ثبت شده؛ اپراتور بانک تازه را خودش اضافه می‌کند |
| **۸** | اپ اندروید فورواردر + جفت‌سازی + heartbeat + تب دستگاه‌ها | خودکار سرتاسری |
| **۹** | گزارش روزانه، هشدارها، پاک‌سازی متن خام | آمادهٔ بهره‌برداری |
