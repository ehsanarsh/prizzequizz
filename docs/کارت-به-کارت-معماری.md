...
## ۱۸-۶) تأیید استقرار تمیز: چه چیزی روی دیتابیس خالی آزمایش شد

هر ۹ مرحله که تمام شد، پیش از هر کار تازه‌ای این را آزمایش کردم: آنچه ساخته
شده، روی یک دیتابیسِ کاملاً خالی درست بالا می‌آید یا نه.

**۱) هر ۳۲ مهاجرت روی دیتابیس خالی اجرا شد** و `db:verify` سبز شد — ۳۴ جدول،
۱۸ ایندکس، صفر مهاجرت معوق.

**۲) دو مسیرِ ساختِ طرح با هم مقایسه شدند.** این پروژه طرح دیتابیس را از دو
جا می‌سازد: مهاجرت‌های شماره‌دار، و `ensure*Schema` که هنگام اجرا خودش جدول را
می‌سازد. دو منبعِ حقیقت برای یک جدول یعنی روزی که با هم فرق کنند. پس یک
دیتابیس فقط با مهاجرت‌ها ساخته شد، یکی فقط با مسیر اجرا، و هفت جدول
کارت‌به‌کارت **ستون‌به‌ستون، ایندکس‌به‌ایندکس و قید‌به‌قید** مقایسه شدند:
۱۶ قید در هر دو، و هیچ اختلافی. مسیر اجرا واقعاً همان چیزی را می‌سازد که
مهاجرت می‌سازد.

**۳) ولی همان مقایسه دو چیز را نشان داد که `db:verify` دنبالشان نمی‌گشت:**

| چه چیزی جا افتاده بود | چرا مهم است |
|---|---|
| جدول `bank_sms_pairings` (مهاجرت ۰۳۲) | شش جدول دیگرِ همین کار در فهرست بودند؛ این یکی فقط چون آخر نوشته شد، نبود |
| ایندکس یکتای `c2c_tracking_unique` | `tryInsertSession` **روی وجودش نوشته شده** و به خطای `23505` همان ایندکس تکیه می‌کند. بدون آن برخورد هرگز خطا نمی‌دهد و دو بازیکن با یک کد پیگیری بیرون می‌آیند |

با فهرست قبلی، دیتابیسی که این جدول و این ایندکس را نداشت **«سالم» گزارش
می‌شد**. این را عملاً آزمایش کردم: جدول و ایندکس را از دیتابیس انداختم،
`db:verify` با فهرست قدیمی `ok = true` داد و با فهرست جدید `exit code = 1`.

**۴) فهرست دیگر به حافظهٔ کسی سپرده نشده.** `schemaContract.test.ts` قاعده را
نگه می‌دارد: هر جدول `c2c_*`/`bank_*` و هر ایندکس یکتای مسیر پول که مهاجرتی
بسازد، باید در فهرست بررسیِ استقرار باشد. جدول تازه‌ای به مسیر پول اضافه کنی و
فهرست را یادت برود، تست **پیش از** استقرار می‌افتد. هر دو پویش نگهبانِ «خالی
نباشد» دارند، چون پویشی که چیزی پیدا نمی‌کند نتیجه‌اش از «همه‌چیز درست است»
قابل تشخیص نیست — و این دقیقاً همان جهشی بود که اول جان به در برد.

> این قاعده عمداً به کل دیتابیس تعمیم داده نشد. خیلی از جدول‌های قدیمی‌تر فقط
> با `ensure*Schema` ساخته می‌شوند و افزودنشان به فهرست، `db:verify` را روی
> دیتابیسی که سالم کار می‌کند قرمز می‌کند. قاعده به مسیر پول محدود است، جایی
> که نبودِ یک جدول یعنی پرداختی که گرفته نمی‌شود.

**۵) یک نکتهٔ عملیاتی که موقع همین آزمایش معلوم شد:** وقتی جدول را از دیتابیس
انداختم و دوباره `npm run migrate` زدم، پیام داد «۳۲/۳۲ مهاجرت اجرا شد» ولی
**هیچ چیزی را برنگرداند** — چون `schema_migrations` آن مهاجرت را «اجراشده»
ثبت کرده. مهاجرت ابزار تعمیر نیست. اگر روزی `db:verify` قرمز شد، راه‌حل
«دوباره migrate بزن» نیست؛ خود اپ موقع اولین استفاده جدول را می‌سازد
(`ensure*Schema`)، و برای ایندکس باید دستی ساخته شود.

---

## ۱۸-۷) هشدار کاتالوگ: از تستی که فروشگاه را دست‌کاری می‌کرد تا تابع خالص

هشدارهای `c2c_nothing_payable` و `c2c_no_payable_ticket` (بند ۱۸-۵) اول با
تستی نوشته شده بودند که **فروشگاه واقعی را غیرفعال و دوباره فعال می‌کرد**. روی
دیتابیس تازه جواب می‌داد و روی دیتابیس مشترک می‌افتاد — و علتِ افتادنش هیچ
ربطی به قاعده‌ای که قرار بود بسنجد نداشت.

منطق به تابع خالص `catalogueAlerts(items, floorToman)` منتقل شد. حالا تست
قیمت‌ها را خودش می‌دهد و روی سه وضعیت دیتابیس (پستگرسِ مشترک، پستگرسِ تمیز، و
بدون دیتابیس) یکسان جواب می‌دهد.

مقایسه عمداً **اکید** است: کالایی که قیمتش دقیقاً برابر کف است، قابل پرداخت
**نیست** — چون `eligibleCards` در `amountToman <= minAmountToman` رد می‌کند. یک
`>=` اینجا فروشگاهی را سالم گزارش می‌کرد که هر بازیکن گزینه‌اش را نمی‌دید. این
با «بلیط قرمز دقیقاً ۵۰٬۰۰۰ است» یک مورد واقعی است، نه فرضی.
