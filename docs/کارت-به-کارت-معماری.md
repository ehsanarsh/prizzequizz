...
| مرحله | محتوا | چرا اول |
|---|---|---|
| **۱ — F2** | جدول بادوام `order_fulfilments` + `money.ts`؛ `_fulfilled: Set` و `_seen: Map` **حذف** می‌شوند | تحویل دوباره را می‌بندد. به هیچ سؤال بازی وابسته نیست، و کارت‌به‌کارت بدون آن اصلاً امن نیست |
...
|---|---|---|
| **۴** | `availability` سه‌حالته + `card_to_card` در `GATEWAY_TYPES` + `c2c_cards` + `amountAllocator` + تب «کارت‌های مقصد» | ادمین کارت تعریف می‌کند؛ درگاه بانکی «به‌زودی» می‌شود |
