...
**وضعیت فعلی: هیچ‌کدام هنوز پاسخ داده نشده‌اند.** این فقط مرحله‌های ۷ و ۸ را نگه
می‌دارد (بند ۱۸)؛ مرحله‌های ۱ تا ۶ به هیچ‌کدام وابسته نیستند و جلو می‌روند. اگر
A1 (ورود مبلغ ریالی در اپ بانکی) منفی از آب دربیاید، تنها اثرش یک سوییچ
`suffixMode` در پنل است و هیچ کدی عوض نمی‌شود؛ اگر A11 (تخفیف) جزو این کار باشد،
روی `quote()` اثر دارد و باید قبل از مرحلهٔ ۵ بدانیم.
...
| **۷** | `bankSms` ingest + پارسرها + `matchService` + `c2cWorker` | تطبیق خودکار روی پیامکی که دستی ثبت شده |
